class ChairsController < ApplicationController
  ...
  # GET /chairs/1.xml
  def show
    @chair = Chair.includes(:teacher).find(params[:id])
    @ch_teachers = Teacher.where(:chair_id => params[:id])
  ...
  def create
    ...
    respond_to do |format|
      if @chair.save
        format.html { redirect_to(chairs_path, :notice => 'Новая кафедра создана.') }
  ...
  def update
    ...
    respond_to do |format|
      if @chair.update_attributes(params[:chair])
        format.html { redirect_to(chair_path, :notice => 'Новая кафедра создана.') }
        format.xml  { head :ok }
      else
